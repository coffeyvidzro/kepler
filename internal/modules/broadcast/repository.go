...
func (r *Repository) Get(ctx context.Context, teamID, id uuid.UUID) (Broadcast, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Broadcast{}, ErrNotFound }
	return value, err
}
...
func (r *Repository) Update(ctx context.Context, teamID, id, segmentID uuid.UUID, topicID *uuid.UUID, templateID uuid.UUID, req UpdateRequest, current Broadcast) (Broadcast, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Broadcast{}, ErrConflict }
	return value, err
}
...
func (r *Repository) ListRecipients(ctx context.Context, teamID, broadcastID uuid.UUID, limit, offset int32) ([]Recipient, error) {
	...
	for rows.Next() {
		var value Recipient
		var snapshot []byte
		if err := rows.Scan(&value.ID,&value.BroadcastID,&value.ContactID,&value.Email,&value.FirstName,&value.LastName,&snapshot,&value.Status,&value.ExclusionReason,&value.EmailMessageID,&value.CreatedAt,&value.QueuedAt); err != nil { return nil, err }
		if err := json.Unmarshal(snapshot, &value.ContactSnapshot); err != nil { return nil, err }
		result = append(result, value)
	}
	return result, rows.Err()
}
