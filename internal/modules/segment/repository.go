...
func (r *Repository) Create(ctx context.Context, teamID uuid.UUID, name string) (Segment, error) {
	...
	if err != nil { return Segment{}, fmt.Errorf("create segment: %w", err) }
	return value, nil
}
...
func (r *Repository) List(ctx context.Context, teamID uuid.UUID, limit, offset int32) ([]Segment, error) {
	...
	if err != nil { return nil, fmt.Errorf("list segments: %w", err) }
	defer rows.Close()
	values := make([]Segment, 0)
	for rows.Next() {
		var value Segment
		if err := rows.Scan(&value.ID, &value.TeamID, &value.Name, &value.CreatedAt); err != nil { return nil, fmt.Errorf("scan segment: %w", err) }
		values = append(values, value)
	}
	if err := rows.Err(); err != nil { return nil, fmt.Errorf("iterate segments: %w", err) }
	return values, nil
}
...
func (r *Repository) ListContacts(ctx context.Context, segmentID, teamID uuid.UUID, limit, offset int32) ([]Contact, error) {
	...
	if err != nil { return nil, fmt.Errorf("list segment contacts: %w", err) }
	defer rows.Close()
	values := make([]Contact, 0)
	for rows.Next() {
		var value Contact
		if err := rows.Scan(&value.ID, &value.TeamID, &value.Email, &value.FirstName, &value.LastName, &value.Unsubscribed, &value.CreatedAt, &value.UpdatedAt); err != nil { return nil, fmt.Errorf("scan segment contact: %w", err) }
		values = append(values, value)
	}
	if err := rows.Err(); err != nil { return nil, fmt.Errorf("iterate segment contacts: %w", err) }
	return values, nil
}
