...
type Handler struct {
	service *backofficecurrencies.Service
}

func NewHandler(service *backofficecurrencies.Service) *Handler {
...
func (handler *Handler) List(c *echo.Context) error {
	...
	page, err := handler.service.List(c.Request().Context(), backofficecurrencies.ListInput{Limit: limit, Offset: offset})
	if err != nil {
...
func (handler *Handler) Create(c *echo.Context) error {
	var input backofficecurrencies.CreateInput
...
func (handler *Handler) Update(c *echo.Context) error {
	var input backofficecurrencies.UpdateInput
	if err := json.NewDecoder(c.Request().Body).Decode(&input); err != nil {
		return httputil.Error(c, apperrors.NewBadRequest("Invalid JSON request body"))
	}
