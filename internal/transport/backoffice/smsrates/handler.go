...
type Handler struct{ service *backofficesmsrates.Service }

func NewHandler(service *backofficesmsrates.Service) *Handler {
...
func (handler *Handler) List(c *echo.Context) error {
	...
	page, err := handler.service.List(c.Request().Context(), backofficesmsrates.ListInput{Limit: limit, Offset: offset})
	if err != nil {
...
func (handler *Handler) Create(c *echo.Context) error {
	var input backofficesmsrates.CreateInput
...
func (handler *Handler) Close(c *echo.Context) error {
	var input backofficesmsrates.CloseInput
	if err := json.NewDecoder(c.Request().Body).Decode(&input); err != nil {
		return httputil.Error(c, apperrors.NewBadRequest("Invalid JSON request body"))
	}
