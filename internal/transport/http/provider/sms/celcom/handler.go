...
func intValue(fields map[string]any, keys ...string) int {
	for _, key := range keys {
		if value, exists := fields[key]; exists {
			...
			case json.Number: parsed, _ := typed.Int64(); return int(parsed)
			}
		}
	}
	return 0
...
func toString(value any) string {
	...
	case float64: return json.Number(strings.TrimRight(strings.TrimRight(fmtFloat(typed), "0"), ".")).String()
	case json.Number: return typed.String()
	default: return ""
	}
}
