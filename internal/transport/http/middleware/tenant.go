	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			if config.Memberships == nil {
				return httputil.Error(c, apperrors.NewInternal("Tenant membership store is not configured", nil))
			}
			...
			membership, err := config.Memberships.GetTenantMembership(c.Request().Context(), teamID, principal.UserID)
			if err != nil || !membership.Active() {
				return httputil.Error(c, apperrors.NewForbidden("Active team membership is required"))
			}
...
func TenantAccess(config TenantAccessConfig) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			...
			if authorization == "" {
				return SessionAuth(SessionAuthConfig{Sessions: config.Sessions, Users: config.Users})(CSRF(config.CSRF)(Tenant(TenantConfig{Memberships: config.Memberships, ParamName: config.TenantParam, HeaderName: config.TenantHeader, Required: config.Required})(next)))(c)
			}
...
func teamIDFromRequest(c *echo.Context, paramName, headerName string) (uuid.UUID, error) {
